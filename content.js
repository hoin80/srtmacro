...
function macrostart() {
	...
	reservSelected = [].map.call(document.querySelectorAll('.reservMacro:checked'), function (select) {
		return select.value;
	});

	if (coachSelected.length == 0 && firstSelected.length == 0 && reservSelected.length == 0) {
		...
		sessionStorage.setItem('reservSelected', JSON.stringify(reservSelected));
...
function macrostop() {
	...
	sessionStorage.removeItem('reservSelected');
...
if (document.URL.substring(0, dsturl1.length) === dsturl1) {
	$(document).ready(function() {
		...
		var reservSelected = JSON.parse(sessionStorage.getItem('reservSelected'));
		if (coachSelected == null) coachSelected = [];
		if (firstSelected == null) firstSelected = [];
		if (reservSelected == null) reservSelected = [];
    ...
    .appendTo("body");
		...
		if ($("#search-list").length != 0) {
			...
			for (i = 1; i < rows.length; i++) {
				...
				if (reserv.children().length > 0) {
					reserv.append($("<p class='p5'></p>"));
					var checkbox = $("<label></label>").html('<input type="checkbox" name="checkbox" class="reservMacro" value="' + i + '"> 매크로');
					checkbox.children('input').prop('checked', reservSelected.indexOf(i+"") > -1);
					reserv.append(checkbox);
				}
			}
		}
		...
		if (sessionStorage.getItem('macro') == "true") {
			...
			if ($("#search-list").length != 0) {
				...
				for (i = 1; i < rows.length; i++) {
					...
					if (reservSelected.indexOf(i+"") > -1) {
						var reservSpecials = reserv.children("a");
						if (reservSpecials.length != 0) {
							for (j = 0; j < reservSpecials.length; j++) {
								name = $(reservSpecials[j]).attr('class');
								if (name == 'btn_small btn_burgundy_dark val_m wx90') {
									$(reservSpecials[0])[0].click();
									succeed = true;
									break;
								}
							}
